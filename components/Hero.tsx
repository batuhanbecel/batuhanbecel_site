...
const socialLinks = [
  ...
  { href: 'https://www.instagram.com/batuhanbecel_/', label: 'Instagram', icon: InstagramIcon },
  { href: 'https://www.linkedin.com/in/batuhanbecel/', label: 'LinkedIn', icon: LinkedInIcon },
]
    ...
    >
      ...
      >
        <motion.p 
          ...
          className="text-[var(--muted)] text-sm md:text-base mb-4 tracking-widest uppercase font-medium"
        >
        ...
        >
          {socialLinks.map(({ href, label, icon: Icon }, index) => (
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className="p-4 glass rounded-xl hover:bg-[var(--foreground)] hover:text-[var(--background)] transition-all duration-300 hover-lift"
              aria-label={label}
            >
              <Icon size={22} />
            </motion.a>
      ...
      <motion.div
        ...
        className="absolute bottom-24 md:bottom-12 left-1/2 -translate-x-1/2"
      >
        <div className="w-7 h-12 border-2 border-[var(--muted)] rounded-full flex justify-center p-1">
          <motion.div
            animate={{ y: [0, 16, 0] }}
            transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
            className="w-2 h-2 bg-[var(--muted)] rounded-full"
          />
        </div>
      </motion.div>
    </section>
